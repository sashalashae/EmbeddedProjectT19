...
void position_tracker(uint16_t FSRs, Position_Data * pdToCpy, int nextPos) {
    ...
    switch (pd.current_position) {
        ...
        case off_inside:
            ...
            if(pd.dir == forwards)
                currentMsg = stringToStruct("{\"MoveCmd\":\"DriveBackward10, TurnRight5\",\"ArmCmd\":\"wait\",\"Beside\":\"9\"}\0", 0);
            else if(pd.dir == reverse)
                currentMsg = stringToStruct("{\"MoveCmd\":\"DriveBackward10, TurnLeft5\",\"ArmCmd\":\"wait\",\"Beside\":\"9\"}\0", 0);
            TxThreadQueue_Send(currentMsg);
            pd.check = 0;
...
void dirTravel(Position_Data * pdToCpy, int nextPos)
{
    ...
    if(nextPos == 0) //bottom left corner
        ...
        {
            if(pd.current_position < 7 || pd.current_position == 8)
                pd.dir = forwards;
...
void toNextLoc(Position_Data * pdToCpy, int nextPos, uint32_t symbol)
{
    ...
    else
    {
        ...
        {
            pd.beside[0] = '9'; // will never match test
            pd.beside[1] = '\0';
        }
        ...
        else if(pd.dir == forwards)
        {
            ...
            {
                /*char temp[MAX_MESSAGE_SIZE];
                temp[0] = (pd.current_position / 10) | 0b110000;
                temp[1] = (pd.current_position % 10) | 0b110000;
                temp[2] = ' ';
                temp[3] = pd.check | 0b110000;
                temp[4] = ' ';
                temp[5] = pd.dir | 0b110000;
                temp[6] = '\0';
                currentMsg = stringToStructValue("{\"CP check dir\":\"$\"}\0",0,temp);
                sleep(400);
                TxThreadQueue_Send(currentMsg);*/
                if(firstCorner)
                {
                ...
                {
                    turnCheck = 0;
                    currentMsg = stringToStructValue("{\"MoveCmd\":\"TurnLeft90, DriveForward10\",\"ArmCmd\":\"wait\",\"Beside\":\"$\"}\0", 0, pd.beside);
                    TxThreadQueue_Send(currentMsg);
                }
            }
            else if(pd.current_position <= 15)
            {
                ...
                temp[0] = (pd.current_position / 10) | 0b110000;
                temp[1] = (pd.current_position % 10) | 0b110000;
                temp[2] = pd.check | 0b110000;
                temp[3] = pd.dir | 0b110000;
                temp[4] = '\0';
                currentMsg = stringToStructValue("{\"$\"}",0,temp);*/
                TxThreadQueue_Send(currentMsg);
            }
        }
        else
        {
            if(pd.current_position == 0 || pd.current_position == 4 || pd.current_position == 8 || pd.current_position == 12)
            {
                ...
                if(turnCheck)
                {
                    turnCheck = 0;
                    firstCorner = 0;
                    currentMsg = stringToStructValue("{\"MoveCmd\":\"TurnRight90, DriveBackward10\",\"ArmCmd\":\"wait\",\"Beside\":\"$\"}\0", 0, pd.beside);
                    TxThreadQueue_Send(currentMsg);   
                }
            }
            else if(pd.current_position <= 15)
