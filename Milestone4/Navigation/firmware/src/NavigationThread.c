...
void TESTTHREAD_Tasks ( void )
{
    ...
    while(1)
    {
        ...
        /*if(fromServer.val2 == 'X')
        {
            sleep(400);
            currentMsg = stringToStruct("{\"RECFROMSERVER\":\"X\"}\0", 0); // actual message doesn't matter for MS4
            TxThreadQueue_Send(currentMsg);
        }
        else if(fromServer.val2 == 'O')
        {
            sleep(400);
            currentMsg = stringToStruct("{\"RECFROMSERVER\":\"O\"}\0", 0); // actual message doesn't matter for MS4
            TxThreadQueue_Send(currentMsg);
        }
        else
        {
            sleep(400);
            currentMsg = stringToStruct("{\"RECFROMSERVER\":\"NADA\"}\0", 0); // actual message doesn't matter for MS4
            TxThreadQueue_Send(currentMsg);
        }
        sleep(400);*/
